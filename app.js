...
function hostLabel(value) { try { return new URL(value).hostname.replace(/^www\./, ""); } catch { return "магазин"; } }
function showNote(text, state = "") { els.formNote.className = `form-note ${state}`.trim(); els.formNote.textContent = text; }
...
function renderProducts(products) {
  els.productGrid.innerHTML = products.map((product) => `
    <article class="product-card">
      ...
      <div class="product-meta"><span class="product-price">${escapeHtml(product.price)}</span><span class="product-badge">${escapeHtml(product.badge)}</span></div>
    </article>`).join("");
...
function renderSpecs() {
  ...
  els.specBody.innerHTML = visibleSpecs.map((spec) => {
    ...
    return `<tr><td class="spec-name">${escapeHtml(spec.label)}${spec.important ? '<span class="importance">важно</span>' : ""}</td><td class="spec-value">${escapeHtml(spec.a)}</td><td class="spec-value">${escapeHtml(spec.b)}</td><td class="spec-diff ${diffClass}">${escapeHtml(spec.diff)}</td></tr>`;
  }).join("");
}

function renderComparison(data) {
  currentData = data; renderProducts(data.products); renderSummary(data.summary); renderSpecs();
...
async function compareEnteredUrls() {
  ...
  try {
    const response = await fetch(`${API_BASE}/api/compare`, {
      method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ urls: [a, b] })
    });
...
function mapApiResponse(payload) {
  return {
    products: payload.products.map((product, index) => ({
      store: product.store || product.source, name: product.title,
      price: formatPrice(product.price), badge: `Товар 0${index + 1}`, url: product.url
    })),
    summary: payload.summary || "Сравнение готово.",
    specs: payload.comparison.map((row) => ({
      key: row.key, label: row.label, a: row.displayValues[0], b: row.displayValues[1],
      diff: row.note, important: row.important,
...
function formatPrice(price) {
  ...
  try { return new Intl.NumberFormat("ru-RU", { style: "currency", currency: price.currency || "RUB", maximumFractionDigits: 0 }).format(price.value); }
  catch { return `${price.value} ${price.currency || ""}`.trim(); }
}

function setLoading(loading) { els.compareButton.disabled = loading; els.compareButton.textContent = loading ? "Сравниваю…" : "Сравнить"; }
...
document.querySelectorAll(".filter").forEach((button) => button.addEventListener("click", () => {
  ...
  button.classList.add("active"); activeFilter = button.dataset.filter; renderSpecs();
}));
[els.urlA, els.urlB].forEach((input) => input.addEventListener("keydown", (event) => { if (event.key === "Enter") compareEnteredUrls(); }));
